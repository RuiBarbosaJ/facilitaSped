import type { NcmOficial, RegraTabelaSped } from "@/tipos/tabelas-receita";
import { ordinalData } from "@/comum/datas";
 ...
 */
export function indexarBase(base: RegraTabelaSped[]): Map<string, RegraTabelaSped[]> {
  const indice = new Map<string, RegraTabelaSped[]>();
  for (const r of base) {
 ...
 */
export function indexarRegrasSemNcm(base: RegraTabelaSped[]): Map<string, RegraTabelaSped[]> {
  const indice = new Map<string, RegraTabelaSped[]>();
  for (const r of base) {
    if (r.ncm || !r.natureza_receita || !r.tabela || !(r.tabela in BENEFICIOS)) continue;
 ...
 */
function regraFraca(r: RegraTabelaSped): boolean {
  return (r.origem === "descricao" && r.ncm.length === 2) || REGEX_EX_TARIFARIO.test(r.descricao);
}

function situacaoDaRegra(r: RegraTabelaSped, hoje: number): "vigente" | "encerrada" | "futura" {
...
function ordenarRegras(a: RegraTabelaSped, b: RegraTabelaSped): number {
 ...
 */
function encontrarRegras(ncm: string, indice: Map<string, RegraTabelaSped[]>, hoje: number): RegrasEncontradas {
  let vigentes: RegraTabelaSped[] = [];
  const alternativas: RegraTabelaSped[] = [];
  const tabelasVistas = new Set<string>();
  let foraDeVigencia: RegraTabelaSped | undefined;
...
export interface ContextoAuditoria {
  base: Map<string, RegraTabelaSped[]>;
  /** Regras de benefício sem NCM, por código de natureza da receita. */
  semNcm: Map<string, RegraTabelaSped[]>;
...
export function auditarLinha(l: LinhaPlanilha, ctx: ContextoAuditoria): LinhaAuditada {
  ...
  if (principal && beneficio) {
    const montar = (r: RegraTabelaSped): RegraSugerida => {
