...
/**
 * Célula que o Excel interpretaria como fórmula.
 *
 * O Excel e o LibreOffice tratam como fórmula qualquer célula que comece com
 * `=`, `+`, `-`, `@`, tabulação ou retorno de carro. Como as planilhas
 * exportadas repetem texto que veio do arquivo do cliente — descrição de
 * produto, observação —, uma descrição começando com `=` viraria código
 * executando na máquina de quem abrisse o relatório.
 */
const INICIO_DE_FORMULA = /^[=+\-@\t\r]/;

/** Prefixo de apóstrofo: o Excel exibe o texto e não o avalia. */
export function sanitizarCelula(valor: unknown): unknown {
...
export async function gerarXlsx(
  ...
  const XLSX = await sheetjs();
  const aba = XLSX.utils.aoa_to_sheet(linhas.map((linha) => linha.map(sanitizarCelula)));
